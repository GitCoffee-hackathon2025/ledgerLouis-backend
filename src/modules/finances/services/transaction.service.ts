...
export const createTransactionService = (
 ...
 async create(userId: ULID, payload: TransactionLedgerBodyType) {
  ...
  // 3. Criação do objeto para o banco de dados (Drizzle)
  const row = {
    ...payload,
    id: generateId(),
    companyId,
  ...
  // 4. Atualização do saldo da conta (Matemática com números)
  ...
  await accountService.updateValue(toId(targetAccount.id), toId(userId), newBalance.toString());
